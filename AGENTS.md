...
`blog`, `modules-reference`, `settings-and-media`, `dev-workflow`. Claude Code
auto-loads them via the `.claude/skills` symlink; other agents should read the
relevant skill before working in that area. Prefer consulting them over
re-deriving conventions from the code.
